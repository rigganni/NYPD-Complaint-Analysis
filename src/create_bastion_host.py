...
def create_bastion_host():
    ...
    response = ec2.run_instances(ImageId='ami-0b1e2eeb33ce3d66f',
                                 MinCount=1,
                                 MaxCount=1,
    ...
    while retry_count <= retries:
        ...
        if result == 0:
            print(
                "Instance is UP & accessible on port 22, the IP address is:  "
    ...
    # Write public DNS address to bastion.cfg
    ...
    config.set("BASTION", "INSTANCE_ID",
               response["Instances"][0]["InstanceId"])

    with open("/tmp/bastion.cfg", "w") as configfile:
